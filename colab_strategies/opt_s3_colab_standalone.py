        ...
        if dir_s3 != 0:
            ...
            if atr > 0 and not np.isnan(atr):
                net_pnl, r_multiple, label, bars_held = simulate_trade(
                    h, l, c, i, entry, atr, dir_s3, tp_mult, trail_atr)
                ...
                flat_trade = {
                    ...
                    'direction': dir_s3, 'net_pnl': net_pnl,
                    'r_multiple': r_multiple, 'label': label
                }
                trades["S3_Trend"].append(flat_trade)
                cd_s3 = i + bars_held + 2
    ...
    try:
        ...
        for tp in TP_MULT_OPTIONS:
            for trail in TRAIL_ATR_OPTIONS:
                ...
                all_combos[(tp, trail)] = {
                    "S3_Trend": pd.DataFrame(trades_std["S3_Trend"]) if len(trades_std["S3_Trend"]) > 0 else pd.DataFrame()
                }
...
def auto_optimize_all():
    ...
    cache_file = os.path.join(cache_dir, "trade_cache_opt_s3_colab_standalone_h288.pkl")
    if os.path.exists(cache_file):
        ...
        for tp in TP_MULT_OPTIONS:
            for trail in TRAIL_ATR_OPTIONS:
                all_trades[(tp, trail)] = {s: {} for s in ["S3_Trend"]}
    ...
    print(f"Trade generation complete. Running S3_Trend...")

    strategies_to_run = ["S3_Trend"]
