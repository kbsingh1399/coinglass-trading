...
def evaluate_signal_fn(mksig):
    ...
    return passed_all, windows_passed, total_pnl, (tot_wn / tot_tr * 100) if tot_tr > 0 else 0

# Wide parameter sweeps
# We want to check:
# 1. Pullback value (p8_val)
# 2. CVD direction (allowing zc20 to be slightly negative, e.g. > -0.5, meaning it's not a severe crash)
# 3. CVD diff (cvd_d >= -1.0 or similar)
# We test 3 key signal formats to see which one passes all 20 windows:

scenarios = [
    # Scenario A: CVD z-score threshold is liberal (e.g., > -0.2)
    {"p8_val": -0.20, "z20_min": -0.2, "z10_min": -0.2, "cvd_d_min": 0},
    {"p8_val": -0.22, "z20_min": -0.2, "z10_min": -0.2, "cvd_d_min": 0},
    {"p8_val": -0.18, "z20_min": -0.2, "z10_min": -0.2, "cvd_d_min": 0},
    # Scenario B: Just CVD momentum turn-up (cvd_d >= 0) without absolute zc20 limits
    {"p8_val": -0.20, "z20_min": -999.0, "z10_min": -999.0, "cvd_d_min": 0},
    {"p8_val": -0.22, "z20_min": -999.0, "z10_min": -999.0, "cvd_d_min": 0},
    {"p8_val": -0.25, "z20_min": -999.0, "z10_min": -999.0, "cvd_d_min": 0},
    # Scenario C: CVD is stronger than BTC CVD (Relative Strength)
    {"p8_val": -0.20, "z20_min": -0.5, "z10_min": -0.5, "cvd_d_min": -999.0},
    {"p8_val": -0.22, "z20_min": -0.5, "z10_min": -0.5, "cvd_d_min": -999.0},
]

# We will also test a grid search over:
# zc20 > min_z, zc10 > min_z
# allowing min_z to be negative (-0.5, -0.3, -0.1, 0.0)
min_z_candidates = [-0.5, -0.4, -0.3, -0.2, -0.1, 0.0]
p8_candidates = [-0.20, -0.22, -0.24, -0.25]
...
for p8_val in p8_candidates:
    for min_z in min_z_candidates:
        def make_signal_test(df):
            ...
            zc10 = df.get("zc10", pd.Series(0, index=df.index)).values
            zc20 = df.get("zc20", pd.Series(0, index=df.index)).values
            
            # Allow CVD to be in pullback but not heavily collapsing
            mask_l = (mc > 0) & (p8 < p8_val) & (zc10 > min_z) & (zc20 > min_z)
            out[mask_l] = 1
            mask_s = (mc < 0) & (p8 > -p8_val) & (zc10 < -min_z) & (zc20 < -min_z)
            out[mask_s] = -1
            return out
            
        passed, win_passed, pnl, wr = evaluate_signal_fn(make_signal_test)
        print(f"GRID: p8_val={p8_val}, min_z={min_z} | Windows Passed: {win_passed}/20 | PnL: ${pnl:,.0f} | WR: {wr:.1f}%")
