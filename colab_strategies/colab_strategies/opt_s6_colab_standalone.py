...
TRAIL_ATR_OPTIONS = [0.4, 0.5, 0.8, 1.0]
...
def generate_features_smc(df, btc_ref):
    df_smc, vwap_feats = prep_vwap(df, btc_ref)
    df_smc, smc_feats = prep_smc(df_smc, btc_ref)
    return df_smc
...
def simulate_trade_vwap_v2(h, l, c, i, entry, atr, best_dir, tp_mult, trail_atr, vwap_array):
    sl_dist = 1.0 * atr
    tp_dist = tp_mult * atr
    initial_sl = entry - sl_dist if best_dir == 1 else entry + sl_dist
    tp = entry + tp_dist if best_dir == 1 else entry - tp_dist
    ...
    for j in range(i + 1, limit):
        ...
        if best_dir == 1:
            if float(l[j]) <= current_sl:
                bars_held = j - i; exit_price = current_sl; break
            if float(h[j]) >= v or float(h[j]) >= tp:
                target_hit = min(v, tp) if (v > entry and tp > entry) else tp
                bars_held = j - i; exit_price = target_hit; break
            if float(h[j]) > best_seen:
                best_seen = float(h[j])
                if (best_seen - entry) >= tp_dist:
                    new_trail = best_seen - trail_dist
            ...
            if float(h[j]) >= current_sl:
                bars_held = j - i; exit_price = current_sl; break
            if float(l[j]) <= v or float(l[j]) <= tp:
                target_hit = max(v, tp) if (v < entry and tp < entry) else tp
                bars_held = j - i; exit_price = target_hit; break
            if float(l[j]) < best_seen:
                best_seen = float(l[j])
                if (entry - best_seen) >= tp_dist:
    ...
    while i < len(df_smc) - 100:
        ...
        if best_dir != 0:
            ...
            net_pnl, r_multiple, label, bars_held = simulate_trade_vwap_v2(
                h, l, c, i, entry, atr, best_dir, tp_mult, trail_atr, vwap)
            ...
            flat_trade.update(feats)
            trades["ML_SMC_Orderflow"].append(flat_trade)
            cd = i + bars_held + 2
        i += 1
    return trades


def process_symbol(symbol, btc_ref_global=None):
    print(f"[{symbol}] Generating trade datasets (S6/SMC only)...", flush=True)
    try:
        df = load_asset(symbol)
        start_time = df.index.min()
        ref = btc_ref_global if symbol != "BTCUSDT" else None
        
        df_smc = generate_features_smc(df.copy(), ref)
        
        all_combos = {}
        for tp in TP_MULT_OPTIONS:
            for trail in TRAIL_ATR_OPTIONS:
                trades_smc = simulate_trades_smc(symbol, df_smc, tp_mult=tp, trail_atr=trail)
                
                all_combos[(tp, trail)] = {
                    "ML_SMC_Orderflow": pd.DataFrame(trades_smc["ML_SMC_Orderflow"]) if "ML_SMC_Orderflow" in trades_smc and len(trades_smc["ML_SMC_Orderflow"]) > 0 else pd.DataFrame()
                }
        ...
        empty = {"ML_SMC_Orderflow": pd.DataFrame()}
...
def build_model(train_df, max_depth=3, learning_rate=0.05, n_estimators=100):
    """Full GOD-Level 4-Model Calibrated Stacking Meta-Learner for Final Window Execution."""
    ...
    if len(train_df) < 10 or len(train_df[train_df['label'] == 1]) < 2 or len(train_df[train_df['label'] == 0]) < 2:
        return None, None
    ...
    lgb_params = dict(max_depth=max_depth, learning_rate=learning_rate, n_estimators=n_estimators,
                      ...
                      device='cpu', min_child_samples=10, subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1)
    
    xgb_params = dict(max_depth=max_depth, learning_rate=learning_rate, n_estimators=n_estimators,
                      scale_pos_weight=scale_pos_weight, random_state=42, n_jobs=1,
                      subsample=0.8, colsample_bytree=0.8, reg_alpha=0.1, verbosity=0)

    rf_xgb_params = dict(max_depth=max_depth+2, n_estimators=1, num_parallel_tree=30, random_state=42,
                         n_jobs=1, subsample=0.8, colsample_bytree=0.8, verbosity=0)

    cat_params = dict(iterations=n_estimators, depth=max_depth, verbose=0, random_seed=42, thread_count=1)

    if USE_GPU:
        xgb_params['tree_method'] = 'hist'
        rf_xgb_params['tree_method'] = 'hist'
    cat_params['thread_count'] = 1

    # Feature Importance Selection: prune bottom 20% features using lightweight LGBM
    ...
    base_rf  = xgb.XGBClassifier(**rf_xgb_params)
    base_cat = CatBoostClassifier(**cat_params)

    n_pos = sum(y_train == 1)
    n_neg = sum(y_train == 0)
    min_class_cnt = min(n_pos, n_neg)
    
    if min_class_cnt < 3:
        base_lgb.fit(X_train_sub, y_train)
        base_xgb.fit(X_train_sub, y_train)
        base_rf.fit(X_train_sub, y_train)
        base_cat.fit(X_train_sub, y_train)
        return [base_lgb, base_xgb, base_rf, base_cat], selected_cols

    cv_folds = min(3, min_class_cnt)
    
    try:
        cal_lgb = CalibratedClassifierCV(base_lgb, method='sigmoid', cv=cv_folds)
        cal_xgb = CalibratedClassifierCV(base_xgb, method='sigmoid', cv=cv_folds)
        cal_rf  = CalibratedClassifierCV(base_rf,  method='sigmoid', cv=cv_folds)
        cal_cat = CalibratedClassifierCV(base_cat, method='sigmoid', cv=cv_folds)
        ...
        meta_learner = LogisticRegression(penalty='l1', solver='liblinear', random_state=42, C=1.0)
        stacking_model = StackingClassifier(estimators=estimators, final_estimator=meta_learner, cv=cv_folds)
        stacking_model.fit(X_train_sub, y_train)
        return stacking_model, selected_cols
    except Exception:
        gc.collect()
        base_lgb.fit(X_train_sub, y_train)
        return base_lgb, selected_cols
...
def build_model_fast(train_df, max_depth=3, learning_rate=0.05, n_estimators=50):
    ...
    lgb_params = dict(max_depth=max_depth, learning_rate=learning_rate, n_estimators=n_estimators,
                      ...
                      device='cpu', max_bin=63, min_child_samples=10, subsample=0.8,
...
REPORT_PATH = ROOT / "smc_oos_report.md"
ARTIFACT_REPORT_PATH = Path(r"C:\Users\SIGMA\.gemini\antigravity\brain\059c9b32-098a-4ccc-ac60-90176704f383\smc_oos_report.md")
FINAL_RESULTS = {}
...
def update_markdown_report():
    ...
    for strategy_name in ["S1_Liquidation", "S2_CVD", "S3_Trend", "ML_SMC_Orderflow"]:
    ...
    if "COMBINED" in FINAL_RESULTS:
        ...
        lines.append(f"- **Total Combined Combined Net Profit (20 Months)**: `${comb['total_net_profit']:,.2f}`")
...
def run_expanding_walk_forward_for_strategy(strategy_name, all_trades_dict):
    print(f"\n==================================================", flush=True)
    print(f"RUNNING 20-WINDOW WALK-FORWARD FOR {strategy_name}", flush=True)
    print(f"==================================================", flush=True)
    ...
    cache_data = {"monthly_details": [], "final_balance": 0, "total_net_profit": 0, "avg_monthly_roi": 0, "total_trades": 0, "overall_wr": 0}
    completed_windows = []
    if cache_json_path.exists():
        try:
            import json as j_mod
            with open(cache_json_path, 'r') as f:
                cache_data = j_mod.load(f)
            completed_windows = [c["start"] for c in cache_data.get("monthly_details", [])]
    ...
    if cache_csv_path.exists():
        try:
            cached_csv_df = pd.read_csv(cache_csv_path)
            cached_csv_df['entry_time'] = pd.to_datetime(cached_csv_df['entry_time'])
            print(f"[{strategy_name}] Loaded CSV cache with {len(cached_csv_df)} trades.")
        except Exception as e:
    ...
    elif strategy_name == "ML_SMC_Orderflow":
        strat_space = [{'t_delta': d} for d in [0.0, 0.2, 0.4, 0.5, 0.8, 1.0, 1.2, 1.5, 2.0]]
    else:  # S3_Trend
    ...
    total_net_profit_acc = sum(d.get('net_pnl', 0) for d in monthly_details)
    ...
    for w_idx, (start_str, end_str) in enumerate(MONTHS):
        if start_str in completed_windows:
            cached_detail = next((d for d in monthly_details if d['start'] == start_str), None)
            if cached_detail is not None:
                if cached_detail['trades'] == 0 or cached_detail['roi_pct'] <= 0:
                    print(f"  [WARN] Cached Window {w_idx+1} was not profitable (Trades: {cached_detail['trades']}, ROI: {cached_detail['roi_pct']:.1f}%). Continuing evaluation...")
        ...
        def evaluate_combo(m_param, s_param, tp_key=default_key, use_fast=False, fixed_prob=None):
            ...
            def process_one_sym(sym):
                if sym not in key_data: return None
                ...
                if strategy_name == "S1_Liquidation":
                    ...
                    filt = lambda df: df[
                        ((df['direction'] == 1) & (df['z_delta'] >= s_param['t_delta'])) |
                        ((df['direction'] == -1) & (df['z_delta'] <= -s_param['t_delta']))
                    ]
                else:
                    filt = lambda df: df[
                ...
                if use_fast:
                    ...
                    if len(strat_prior) < 20:
                        strat_prior = filt(prior_df)
                else:
                    strat_prior = filt(prior_df) if len(prior_df) > 0 else prior_df
                    strat_test = filt(test_df)
                
                if len(strat_test) == 0: return None
            ...
            if use_fast:
                for sym in SYMBOLS:
                    res = process_one_sym(sym)
                    if res is not None:
                        sym_predictions.append(res)
            else:
                with ThreadPoolExecutor(max_workers=min(len(SYMBOLS), os.cpu_count() or 4)) as _pool:
                    futures = {_pool.submit(process_one_sym, sym): sym for sym in SYMBOLS}
            ...
            if fixed_prob is not None:
                cand_df = all_sym_preds[all_sym_preds['prob'] >= fixed_prob].copy()
                n_cand = len(cand_df)
                if n_cand > 0:
                    prob_factor = 0.5 + np.clip((cand_df['prob'].values - 0.5) * 2.0, 0.0, 1.0)
                    # Use a default risk multiplier of 1.0 since we don't track best_rmult
                    kelly_mult = prob_factor * 1.0
                    adj_pnl = cand_df['net_pnl'].values * kelly_mult
                    cand_df['adj_pnl'] = adj_pnl
                    
                    n_wins = np.sum(adj_pnl > 0)
                    cand_wr = (n_wins / n_cand) * 100.0
                    cand_pnl = np.sum(adj_pnl)
                    cand_roi = (cand_pnl / INITIAL_CAPITAL) * 100.0
                    cand_cum_pnl = np.cumsum(adj_pnl)
                    cand_equity = INITIAL_CAPITAL + cand_cum_pnl
                    cand_peak = np.maximum.accumulate(cand_equity)
                    cand_dd = np.max((cand_peak - cand_equity) / cand_peak * 100.0)
                    passed = (cand_wr >= 0.0 and cand_roi >= -5.0 and cand_dd < 50.0)
                    
                    local_best_df = cand_df
                    local_found_pass = passed
                    local_best_score = (cand_roi / max(cand_dd, 1.0)) * np.log1p(n_cand)
                return local_best_df, local_found_pass, local_best_score, fixed_prob
                
            all_probs = all_sym_preds['prob'].values.astype(np.float64)
            all_pnls = all_sym_preds['net_pnl'].values.astype(np.float64)
            n_total = len(all_probs)
            if n_total == 0:
                return local_best_df, local_found_pass, local_best_score, local_best_prob

            sort_idx = np.argsort(all_probs)[::-1]
            sorted_probs = all_probs[sort_idx]
            sorted_pnls = all_pnls[sort_idx]

            for perc in prob_percentiles:
                n_cand = max(1, int(np.round((1.0 - perc) * n_total)))
                sub_probs = sorted_probs[:n_cand]
                sub_pnls = sorted_pnls[:n_cand]
                prob_factor = 0.5 + np.clip((sub_probs - 0.5) * 2.0, 0.0, 1.0)

                for r_mult in risk_multipliers:
                    kelly_mult = prob_factor * r_mult
                    adj_pnl = sub_pnls * kelly_mult
                    ...
                    if passed:
                        ...
                        if not local_found_pass or score > local_best_score:
                            cand_df = all_sym_preds.iloc[sort_idx[:n_cand]].copy()
                            cand_df['adj_pnl'] = adj_pnl
                            local_best_df = cand_df
                            local_found_pass = True
                            local_best_score = score
                            local_best_prob = sorted_probs[n_cand - 1]
        ...
        n_parallel_jobs = min(os.cpu_count() or 2, 2)
        print(f"  Optuna TPE search (60 trials x {n_parallel_jobs} workers + Stacking Meta-Learner [LGB/XGB/RF/Cat, CPU] + Kelly Sizing) across symbols...", flush=True)
        ...
        study.optimize(optuna_objective, n_trials=60, n_jobs=n_parallel_jobs, show_progress_bar=False)
        ...
        t_df, t_pass, _, _ = evaluate_combo(t_m, t_s, tp_key=t_tp_key, use_fast=False, fixed_prob=best_val_prob)
        gc.collect()
        if t_pass and not t_df.empty:
            ...
            print(f"    -> Strat Params: {t_s}")
            print(f"    -> ML Params: {t_m}")
        ...
        if best_window_df.empty or not found_pass:
            ...
            if best_window_df.empty:
                ...
                _tw = len(best_window_df[best_window_df['adj_pnl'] > 0]) if 'adj_pnl' in best_window_df.columns else 0
                _tr = len(best_window_df)
                _wr = (_tw / _tr * 100) if _tr > 0 else 0.0
                _roi = (best_window_df['adj_pnl'].sum() / INITIAL_CAPITAL * 100) if 'adj_pnl' in best_window_df.columns else 0.0
                print(f"  Using best available result: WR={_wr:.1f}%, ROI={_roi:.1f}%, Trades={_tr}")
            
        w_trades_cnt = len(best_window_df)
        w_wins = len(best_window_df[best_window_df['adj_pnl'] > 0]) if w_trades_cnt > 0 else 0
        w_wr = (w_wins / w_trades_cnt * 100) if w_trades_cnt > 0 else 0.0
        w_net_pnl = best_window_df['adj_pnl'].sum() if w_trades_cnt > 0 else 0.0
        w_roi_pct = (w_net_pnl / INITIAL_CAPITAL) * 100
        
        w_cum_pnl = best_window_df['adj_pnl'].cumsum() if w_trades_cnt > 0 else pd.Series([0])
        ...
        # Incremental Save after every window to prevent loss on interrupt
        try:
            ...
            # Incremental CSV append — keep only essential columns to save RAM
            keep_cols = ['symbol', 'entry_time', 'direction', 'net_pnl', 'adj_pnl', 'label']
            slim_df = best_window_df[[c for c in keep_cols if c in best_window_df.columns]].copy()
            write_header = not cache_csv_path.exists() or cache_csv_path.stat().st_size == 0
            slim_df.to_csv(cache_csv_path, mode='a', header=write_header, index=False)
            print(f"[{strategy_name}] Incremental cache saved (Window {w_idx+1}/20).")
        except Exception as e_save:
            print(f"[WARN] Incremental save failed: {e_save}")

        # Aggressively free window-local memory to prevent OOM on Colab
        del window_sym_data, best_window_df, study
        gc.collect()
    ...
    FINAL_RESULTS[strategy_name] = {
        ...
        'total_net_profit': float(total_net_profit_acc),
        'avg_monthly_roi': float(avg_monthly_roi),
        'total_trades': int(total_trades_count),
        'overall_wr': float(overall_wr),
        'oos_df': pd.read_csv(cache_csv_path) if cache_csv_path.exists() else pd.DataFrame()
    }
...
def _flatten_symbol_trades(sym, btc_ref_global=None):
    _, all_combos, _ = process_symbol(sym, btc_ref_global=btc_ref_global)
    result = {}
...
def auto_optimize_all():
    ...
    cache_file = os.path.join(cache_dir, "trade_cache_opt_s6_colab_standalone_h288.pkl")
    if os.path.exists(cache_file):
        ...
        print(f"Generating base trades for {len(SYMBOLS)} symbols x {len(TP_MULT_OPTIONS)} TP x {len(TRAIL_ATR_OPTIONS)} Trail combos...", flush=True)

        print("Loading BTC reference data (once)...", flush=True)
        btc = load_asset("BTCUSDT")
        btc_ref_global = btc[["Close", "CVD"]].copy()
        btc_ref_global.columns = ["btc_Close", "btc_CVD"]
        del btc
        gc.collect()
        ...
        for tp in TP_MULT_OPTIONS:
            for trail in TRAIL_ATR_OPTIONS:
                all_trades[(tp, trail)] = {s: {} for s in ["S1_Liquidation", "S2_CVD", "S3_Trend", "ML_SMC_Orderflow"]}

        print("Processing symbols sequentially (Safe Mode for Colab 12GB RAM)...", flush=True)
        for sym in SYMBOLS:
            sym_result = _flatten_symbol_trades(sym, btc_ref_global)[1]
            print(f"  [DONE] {sym} base trade dataset ready.", flush=True)
    ...
    print(f"Trade generation complete. Running ML_SMC_Orderflow walk-forward...", flush=True)

    strategies_to_run = ["ML_SMC_Orderflow"]
