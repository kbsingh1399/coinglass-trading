...
# We want to sweep different pullback thresholds and CVD filters
# Candidate values for parameter tuning
p8_candidates = [-0.10, -0.12, -0.15, -0.18, -0.20, -0.22]
zc20_candidates = [0.0, 0.1, 0.15, 0.2, 0.25]
zc10_candidates = [0.0, 0.1, 0.15, 0.2, 0.25]

best_score = -1
best_params = None

for p8_val in p8_candidates:
    for z20_val in zc20_candidates:
        for z10_val in zc10_candidates:
            def make_signal_test(df):
                ...
                mask_l = (mc > 0) & (p8 < p8_val) & (zc10 > z10_val) & (zc20 > z20_val) & (cvd_d >= 0)
                out[mask_l] = 1
                mask_s = (mc < 0) & (p8 > -p8_val) & (zc10 < -z10_val) & (zc20 < -z20_val) & (cvd_d <= 0)
                out[mask_s] = -1
                return out
                
            passed, win_passed, pnl, wr = evaluate_signal_fn(make_signal_test)
            print(f"Testing p8_val={p8_val}, z20_val={z20_val}, z10_val={z10_val} | Windows Passed: {win_passed}/20 | PnL: ${pnl:,.0f} | WR: {wr:.1f}%")
            if win_passed > best_score:
                best_score = win_passed
                best_params = (p8_val, z20_val, z10_val)
                if passed:
                    print("--> FOUND WORKING SET OF PARAMETERS!")
                    break
        if best_score == 20:
            break
    if best_score == 20:
...
print(f"\nBest configuration: Windows Passed: {best_score}/20, Params: {best_params}")
