...
min_z_candidates = [-0.6, -0.4, -0.2, 0.0]
p8_candidates = [-0.15, -0.20, -0.25]

# Let's try some specific CVD conditions.
# If CVD is in uptrend or is at least not heavily falling.
# Also let's try Relative CVD strength (CVD zscore > btc_CVD zscore) - this is a classic quant trick!
# "zc20 > zb20" (Asset CVD is stronger than BTC CVD)

modes = ["CVD_Zscore", "CVD_vs_BTC_CVD", "CVD_Diff_Only"]
    ...
    for p8_val in p8_candidates:
        for min_z in min_z_candidates:
            def make_signal_test(df):
                ...
                if mode == "CVD_Zscore":
                    mask_l = (mc > 0) & (p8 < p8_val) & (zc20 > min_z)
                    mask_s = (mc < 0) & (p8 > -p8_val) & (zc20 < -min_z)
                elif mode == "CVD_vs_BTC_CVD":
                    # Relative strength: Asset CVD z-score must be higher than BTC CVD z-score for longs
                    mask_l = (mc > 0) & (p8 < p8_val) & (zc20 > zb20 + min_z)
                    mask_s = (mc < 0) & (p8 > -p8_val) & (zc20 < zb20 - min_z)
                else: # CVD_Diff_Only
                    # cvd_d is rolling difference. min_z scaled
                    mask_l = (mc > 0) & (p8 < p8_val) & (cvd_d > min_z * df["atr"])
                    mask_s = (mc < 0) & (p8 > -p8_val) & (cvd_d < -min_z * df["atr"])
            ...
            print(f"Mode={mode}, p8={p8_val}, z={min_z} | Passed: {win_passed}/20 | PnL: ${pnl:,.0f} | Reason: {fail_reason}")
            if win_passed > best_score:
                best_score = win_passed
                best_params = (mode, p8_val, min_z)
                if passed:
