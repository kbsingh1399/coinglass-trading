...
def find_script(script_path: str, project_path: Path, agent_dir_name: str) -> Optional[Path]:
    """
    Dynamically locate the verification script:
    1. Local relative path (in the active workspace)
    2. Sibling ag-kit repository (.agents folder)
    3. Global ~/.gemini/config/skills folder
    """
    p_path = Path(script_path.replace(".agents", agent_dir_name))
    local_path = project_path / p_path
    if local_path.exists() and local_path.is_file():
        return local_path
        
    ag_kit_base = project_path.parent / "ag-kit"
    if ag_kit_base.exists():
        sibling_path = ag_kit_base / p_path
    ...
    if len(parts) >= 3:
        skill_name = parts[2]
        script_subpath = Path(*parts[3:]) if len(parts) > 3 else Path()
        global_path = Path("C:/Users/SIGMA/.gemini/config/skills") / skill_name / script_subpath
        if global_path.exists() and global_path.is_file():
            return global_path
            
    return None

def run_script(name: str, script_path: Optional[Path], project_path: str, url: Optional[str] = None) -> dict:
...
def print_final_report(results: List[dict], start_time: datetime):
    ...
    # Final verdict
    failed_required = sum(1 for r in results if not r["passed"] and not r.get("skipped") and r.get("required", True))
    if failed_required > 0:
        print_error(f"VERIFICATION FAILED - {failed_required} critical check(s) need attention")
        print(f"\n{Colors.YELLOW}💡 Tip: Fix critical (security, lint) issues first{Colors.ENDC}")
        return False
    else:
        print_success("✨ ALL REQUIRED CHECKS PASSED - Ready for deployment! ✨")
        return True
  ...
  python scripts/verify_all.py . --url https://staging.example.com --no-e2e
    ...
    for suite in VERIFICATION_SUITE:
        ...
        for name, script_path, required in suite["checks"]:
            script = find_script(script_path, project_path, agent_dir_name)
