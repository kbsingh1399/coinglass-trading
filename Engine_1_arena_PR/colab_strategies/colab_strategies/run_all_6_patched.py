...
def make_signal_s2(df):
    """S2: CVD Momentum — FIXED: relative strength to BTC CVD
    
    Now: trend pullback + asset CVD z-score relative strength vs BTC CVD
    """
    out=np.zeros(len(df),dtype=np.int32)
    mc=df.get("mc",pd.Series(0,index=df.index)).values
    p8=df.get("p8",pd.Series(0,index=df.index)).values
    ...
    # Long: uptrend + pullback + CVD z-score stronger than BTC CVD z-score - 0.08
    mask_l=(mc>0)&(p8<-0.22)&(zc20>zb20-0.08)
    out[mask_l]=1
    # Short: downtrend + bounce + CVD z-score weaker than BTC CVD z-score + 0.08
    mask_s=(mc<0)&(p8>0.22)&(zc20<zb20+0.08)
    out[mask_s]=-1
    return out

def make_signal_s3(df):
    """S3: Pure trend pullback (UNCHANGED)"""
    out=np.zeros(len(df),dtype=np.int32)
    mc=df.get("mc",pd.Series(0,index=df.index)).values
    p8=df.get("p8",pd.Series(0,index=df.index)).values
...
def run_one(name,mksig):
    ...
    for wi,(ss,se) in enumerate(MONTHS):
        ...
        nt=len(bdf)
        if nt==0: log(f"    No trades after filter"); res.append({'w':wi+1,'start':ss,'end':se,'tr':0,'wins':0,'wr':0,'pnl':0,'roi':0,'dd':0,'mtm_dd':0,'passed':False}); break
...
if __name__=="__main__":
    ...
    with open('patched_results.json','w') as f: json.dump({k:[{kk:str(vv) for kk,vv in r.items()} for r in v] for k,v in all_res.items()},f,indent=2,default=str)
    log("Saved: patched_results.json")
