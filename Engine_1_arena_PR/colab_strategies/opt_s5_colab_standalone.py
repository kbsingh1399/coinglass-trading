...
USE_GPU = True
print(f'[GPU] LightGBM GPU mode: {"ENABLED" if USE_GPU else "DISABLED (CPU fallback)"}', flush=True)
...
def generate_features_vwap(df, btc_ref):
    ...
    df_vwap = prep_microstructure(df_vwap)
    return df_vwap
...
def simulate_trade_vwap_v2(h, l, c, i, entry, atr, best_dir, trail_atr, tp_mult, vwap_array):
    sl_dist = 1.0 * atr
    tp_dist = tp_mult * atr
    initial_sl = entry - sl_dist if best_dir == 1 else entry + sl_dist
    trail_dist = trail_atr * atr
    ...
    while i < len(df_vwap) - 100:
        ...
        if best_dir != 0:
            ...
            net_pnl, r_multiple, label, bars_held = simulate_trade_vwap_v2(
                h, l, c, i, entry, atr, best_dir, trail_atr, tp_mult, vwap)
            feats = {col: feat_arrs[col][i] for col in feat_cols}
            actual_entry_time = ts[i+1] if i + 1 < len(ts) else ts[i]
            exit_bar_idx = min(i + bars_held, len(ts) - 1)
            actual_exit_time = ts[exit_bar_idx]
            ...
            flat_trade.update(feats)
            trades["ML_Microstructure"].append(flat_trade)
            cd = i + bars_held + 2
        i += 1
    return trades
...
def process_symbol(symbol, btc_ref_global=None):
    print(f"[{symbol}] Generating trade datasets...", flush=True)
    try:
        ...
        df_vwap = generate_features_vwap(df.copy(), ref)
        ...
        for tp in TP_MULT_OPTIONS:
            for trail in TRAIL_ATR_OPTIONS:
                ...
                trades_vwap = simulate_trades_microstructure(symbol, df_vwap, tp_mult=tp, trail_atr=trail)
                
                trades = trades_std
                for k, v in trades_vwap.items():
                    if k not in trades or len(trades[k]) == 0:
                        trades[k] = v
                
                all_combos[(tp, trail)] = {}
                for strat, tr_list in trades.items():
                    if len(tr_list) > 0:
                        df_tr = pd.DataFrame(tr_list)
                        fcols = df_tr.select_dtypes(include=['float64']).columns
                        if len(fcols) > 0:
                            df_tr[fcols] = df_tr[fcols].astype(np.float32)
                        all_combos[(tp, trail)][strat] = df_tr
                    else:
                        all_combos[(tp, trail)][strat] = pd.DataFrame()
                        
        del df_std, df_vwap, df
        gc.collect()
        return symbol, all_combos, start_time
    except Exception as e:
        print(f"Error on {symbol}: {e}", flush=True)
        import traceback; traceback.print_exc()
        empty = {s: pd.DataFrame() for s in ["S1_Liquidation", "S2_CVD", "S3_Trend", "ML_Microstructure"]}
...
class SimpleEnsembleClassifier:
    ...
    def fit(self, X, y):
        ...
        try:
            ...
            self.cat_model.set_params(task_type='CPU', thread_count=1)
            self.cat_model.fit(X, y)
        return self
...
def build_model(train_df, max_depth=3, learning_rate=0.05, n_estimators=100):
    ...
    if len(train_df) < 10 or len(train_df[train_df['label'] == 1]) < 2 or len(train_df[train_df['label'] == 0]) < 2: return None, None
...
def build_model_fast(train_df, max_depth=3, learning_rate=0.05, n_estimators=100):
    ...
    # LightGBM uses CPU OpenMP multi-threading (n_jobs=-1) for rock-solid stability across parallel threads
    pass
...
def update_markdown_report():
    ...
    for strategy_name in ["S1_Liquidation", "S2_CVD", "S3_Trend", "ML_Microstructure"]:
...
def run_expanding_walk_forward_for_strategy(strategy_name, all_trades_dict):
    ...
    elif strategy_name == "ML_Microstructure":
        strat_space = [
            {'t_vol': v, 't_delta': d}
            for v in [-1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 5.0, 99.0]
    ...
    ml_space = [
        {'max_depth': 2, 'learning_rate': 0.10, 'n_estimators': 50},
        ...
        {'max_depth': 6, 'learning_rate': 0.005, 'n_estimators': 500},
    ]
    ...
    monthly_details = []
    strategy_oos_dfs = []  # kept for legacy compatibility but not accumulated in-memory
    cache_data = {"monthly_details": [], "final_balance": 0, "total_net_profit": 0, "avg_monthly_roi": 0, "total_trades": 0, "overall_wr": 0}
    ...
    for w_idx, (start_str, end_str) in enumerate(MONTHS):
        ...
        def evaluate_combo(m_param, s_param, tp_key=default_key, use_fast=False, fixed_prob=None):
            ...
            def _process_sym(sym):
                ...
                if strategy_name == "S1_Liquidation":
                    ...
                    filt = lambda df, _sp=s_param: df[
                        (df['vol_regime'] <= _sp['t_vol']) & (
                            ((df['direction'] == 1) & (df['delta_qty_z'] >= _sp['t_delta'])) |
                            ((df['direction'] == -1) & (df['delta_qty_z'] <= -_sp['t_delta']))
                        ) if 'delta_qty_z' in df.columns else
                        (df['vol_regime'] <= _sp['t_vol'])
                    ]
                else:
                    filt = lambda df: df[
        ...
        n_parallel_jobs = 2
        n_trials = 75 if strategy_name == "ML_Microstructure" else 50
        print(f"  Optuna TPE search ({n_trials} trials x {n_parallel_jobs} workers + Stacking Meta-Learner [LGB/XGB/RF/Cat] + Kelly Sizing, GPU=ON) across symbols...", flush=True)
        ...
        study.optimize(optuna_objective, n_trials=n_trials, n_jobs=n_parallel_jobs, show_progress_bar=False)
...
def compute_combined_portfolio():
    ...
    all_strat_dfs = []
    for strat in ["ML_Microstructure"]:
...
def auto_optimize_all():
    ...
    cache_file = os.path.join(cache_dir, "trade_cache_opt_s5_colab_standalone_h288.pkl")
    if os.path.exists(cache_file):
        ...
        for tp in TP_MULT_OPTIONS:
            for trail in TRAIL_ATR_OPTIONS:
                all_trades[(tp, trail)] = {s: {} for s in ["S1_Liquidation", "S2_CVD", "S3_Trend", "ML_Microstructure"]}

        print("Processing symbols sequentially (Numba deadlock-free mode)...", flush=True)
        for sym in SYMBOLS:
    ...
    print(f"Trade generation complete. Running ML_Microstructure walk-forward...", flush=True)

    strategies_to_run = ["ML_Microstructure"]
    ...
    print("\n20-Window Expanding Walk-Forward Optimization completed successfully.", flush=True)
